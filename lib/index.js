...
class Timesheet extends React.Component {
  ...
  validate(dest) {
    ...
    return !!day.find((schedule) =>
      ...
      dest.start.isBefore(schedule.start) &&
      dest.start.isAfter(schedule.end) &&
      // Must end before and after end
      dest.end.isBefore(schedule.start) &&
      dest.end.isAfter(schedule.end));
  }
  ...
  handleDragStop(evt) {
    ...
    if ( !time.start.isSame(schedule.start) ) {
      ...
      schedule.start = time.start.clone();
